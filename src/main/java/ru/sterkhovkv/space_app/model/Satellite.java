...
public class Satellite {
    ...
    // Метод конвертации из SpaceStation в Satellite
    public static Satellite fromSpaceStation(SpaceStation spaceStation) {
        return Satellite.builder()
                //.id(spaceStation.getId())
                .objectName(spaceStation.getObjectName())
                .epoch(spaceStation.getEpoch())
                .meanMotion(spaceStation.getMeanMotion())
                .eccentricity(spaceStation.getEccentricity())
                .inclination(spaceStation.getInclination())
                .raOfAscNode(spaceStation.getRaOfAscNode())
                .argOfPericenter(spaceStation.getArgOfPericenter())
                .meanAnomaly(spaceStation.getMeanAnomaly())
                .classificationType(spaceStation.getClassificationType())
                .noradCatId(spaceStation.getNoradCatId())
                .revAtEpoch(spaceStation.getRevAtEpoch())
                .bstar(spaceStation.getBstar())
                .meanMotionDot(spaceStation.getMeanMotionDot())
                .meanMotionDdot(spaceStation.getMeanMotionDdot())
                .visible(spaceStation.getVisible())
                .tleLine1(spaceStation.getTleLine1())
                .tleLine2(spaceStation.getTleLine2())
                .build();
    }
    ...
    public static Satellite fromTLE(SatelliteTLEDTO satelliteTLEDTO) {
        String line1 = satelliteTLEDTO.getLine1();
        String line2 = satelliteTLEDTO.getLine2();

        double nddot = gdi(line1.charAt(44),line1,45,50, 0);
        nddot *= Math.pow(10.0, gd(line1,50,52, 0));
        double bstar = gdi(line1.charAt(53),line1,54,59, 0);
        bstar *= Math.pow(10.0d, gd(line1,59,61, 0));

        return Satellite.builder()
                .objectName(satelliteTLEDTO.getName())
                .epoch(parseEpoch(line1.substring(18,32).trim()))
                .meanMotion(gd(line2,52,63,0))
                .eccentricity(gdi('+',line2,26,33,0))
                .inclination(gd(line2,8,16,0))
                .raOfAscNode(gd(line2,17,25,0))
                .argOfPericenter(gd(line2,34,42,0))
                .meanAnomaly(gd(line2,43,51,0))
                .classificationType(String.valueOf(line1.charAt(7)))
                .noradCatId(Integer.parseInt(line1.substring(2,7).trim()))
                .revAtEpoch((int)gd(line2,63,68,0))
                .bstar(bstar)
                .meanMotionDot(gdi(line1.charAt(33),line1,35,44, 0))
    ...
    private static double gdi(char sign, String str, int start, int end, double defVal)
    {
        double num = defVal;
    ...
    private static double gd(String str, int start, int end, double defVal)
    {
        double num = defVal;
